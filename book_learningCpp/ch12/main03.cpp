...
struct SortData<T*>
{
    static void Sort(std::vector<T*>& data)
    {
        ...
        std::cout << "Using template specialization SortData() for pointers" << std::endl;
    }
};
...
int main()
{
    ...
    SortData<int*>::Sort(pointers); // notice the use of int* here, calling the specialization
    std::cout << "Sorted pointers: ";
    for (const auto& ptr : pointers)
    {
        std::cout << *ptr << " ";
        delete ptr; // delete allocated memory as we go
    }
    std::cout << std::endl;
